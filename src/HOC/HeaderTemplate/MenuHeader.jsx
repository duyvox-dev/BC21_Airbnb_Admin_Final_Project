  ...
  return (
    <div>
      <Menu as="div" className="relative inline-block text-left">
        <div>
          <Menu.Button className="w-20 h-10 rounded-3xl p-2 bg-white text-sm font-medium text-black hover:shadow-lg border border-stone-400 transition-all duration-300 flex justify-between items-center">
            ...
            {adminLogin ? (
              ...
              <FontAwesomeIcon className="w-8 h-8" icon={faCircleUser} />
            )}
          </Menu.Button>
        </div>
        <Transition
        ...
        >
          <Menu.Items
            className="absolute -right-1/4 mt-2 w-28 h-8 origin-top-right divide-y divide-gray-100 rounded-md bg-white shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none flex justify-center items-center cursor-pointer hover:bg-gray-300 transition-all duration-300"
            onClick={() => dispatch(dangXuat())}
          >
            <Menu.Item>
              <div className="text-lg font-normal">Đăng xuất</div>
            </Menu.Item>
          </Menu.Items>
        </Transition>
      </Menu>
