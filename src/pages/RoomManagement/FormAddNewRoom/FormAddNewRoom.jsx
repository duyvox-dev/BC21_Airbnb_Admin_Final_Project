...
export default function FormAddNewRoom() {
    ...
    const renderLocationProvinceList = () => {
        ...
        return locationList.map((location, index) => {
            return (
                <Select.Option key={index} value={location.province}>
                    {location.province}
                </Select.Option>
            );
        });
    ...
    const renderLocationNameList = () => {
        //Render location name according to chosen province
        return locationList
            .filter((location) => location.province === locationProvince)
            .map((location, index) => {
                return (
    ...
    const handleLocationProvinceChange = (value) => {
        //Get the province selection
        setLocationProvince(value);
    };

    const handleLocationNameChange = (value) => {
        //Get the location name selection & find locationId
        ...
        if (indexLocationName !== -1) {
            formik.setFieldValue(
                "locationId",
                locationList[indexLocationName]._id
            );
        }
    };
            ...
            >
                ...
                <div className="w-full flex justify-between">
                    ...
                    <Form.Item label="Số lượng nhà tắm">
                        <InputNumber
                            onChange={(value) => {
                                formik.setFieldValue("bath", value);
                            }}
                    ...
                    <Form.Item label="Số lượng phòng">
                        <InputNumber
                            onChange={(value) => {
                                formik.setFieldValue("bedRoom", value);
                            }}
